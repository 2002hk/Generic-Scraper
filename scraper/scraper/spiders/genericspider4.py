...
class Genericspider4Spider(scrapy.Spider):
    name = "genericspider4"
    ...
    def __init__(self,start_date=None,end_date=None,start_url=None,custom_set=None,fetch_method=None,fetch_decisions=None,delay_sec=None,
        ...
        super(Genericspider4Spider,self).__init__(*args,**kwargs)
    ...
    def parse(self,response):
        ...
        for link in links:
            ...
            self.default_delay()
            details_page_url=self.details_url % (self.start_url, appKey)
            self.driver.get(details_page_url)
            details_rows=self.driver.find_elements(By.XPATH,'//tr')
            ...
            contact_page_url=self.contacts_url % (self.start_url, appKey)
            self.default_delay()
            self.driver.get(contact_page_url)
            try:
                message=self.driver.find_element(By.XPATH,'//div[@class="tabcontainer"]/p').text
                if message=='There are no contacts associated with this case.':
                    print('No contacts to scrape')
            except:

            
                
                agent_name = self.driver.find_element(By.XPATH,'//div[@class="agents"]/p').text
                ...
                try: 
                    table = self.driver.find_element(By.XPATH,'//table[@class="agents"]')
                    table_row=table.find_elements(By.XPATH,'.//tr')
                    for row in table_row:
                        ...
                        if header.upper()=='Phone'.upper() or header.upper()=='Home Phone Number'.upper() \
                                ...
                                or header.upper()=='Telephone number'.upper() or header.upper()=='Phone Number'.upper():
                            value = row.find_element(By.XPATH,'.//td').text
                            value = self.text_replace(value,'\n','')
                            value = self.text_strip(value)
    ...
    def check_and_yield(self, data):
        """Only yield if all fields are filled"""
