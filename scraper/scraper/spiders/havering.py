...
class HaveringSpider(scrapy.Spider):
    ...
    def __init__(self,start_date=None,end_date=None,start_url=None,custom_set=None,fetch_method=None,fetch_decisions=None,delay_sec=None,
        ...
        if start_url:
            ...
            self.start_url='https://development.havering.gov.uk/OcellaWeb/planningSearch'
    ...
    def start_requests(self):
        ...
        yield SeleniumRequest(url=scrape_url,callback=self.doSearch,implicitly_wait=120,time_sleep_millisec=6000)
    ...
    def doSearch(self,response):
        ...
        time.sleep(10)
        self.driver.find_element(By.XPATH,'//input[@value="Show all results"]').click()
        time.sleep(10)
    ...
    def parse(self,response):
        #appItem=HaveringItem()
        tables=response.xpath('//table')
        print('the len of tables', len(tables))
        actual_table=tables[1].xpath('./tbody/tr')[1:]
        print('***actual table**',actual_table)
        for row in actual_table:
            cols=row.xpath('./td')
            reference=cols[0].xpath('./a/text()').get()
            main_page_url=self.result_page_url % (self.start_url,reference)
            yield SeleniumRequest(url=main_page_url,callback=self.scrapemainpage,implicitly_wait=120,time_sleep_millisec=6000)
            
    def scrapemainpage(self,response):
        
        appItem=HaveringItem()
        appItem['reference']=response.xpath(" //tr[td[strong[text()='Reference']]]/td[2]/text()").get()
        appItem['location']=response.xpath("//tr[td[strong[text()='Location']]]/td[2]/text()").get()
        appItem['proposal']=response.xpath("//tr[td[strong[text()='Proposal']]]/td[2]/text()").get()
        appItem['status']=response.xpath("//tr[td[strong[text()='Status']]]/td[2]/text()").get()
